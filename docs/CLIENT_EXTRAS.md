# Client extras — `amana-union-cargills-bank-docs`

## Typed models

Canonical dataclass / TS types from field-coverage domains: `fx_tt`, `fd_deposits`, `card_offers`.

- Python: `python/amana_union_cargills_bank_docs/models.py`
...
```python
from amana_union_cargills_bank_docs import AmanaUnionCargillsBankDocsClient, iter_lab_endpoint

with AmanaUnionCargillsBankDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { AmanaUnionCargillsBankDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new AmanaUnionCargillsBankDocsClient();
...
```python
from amana_union_cargills_bank_docs import shard_groups, shard_page_numbers, shard_offsets
